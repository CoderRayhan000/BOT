...
module.exports = {
    ...
    async execute(interaction, message, client) {
        ...
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) return interaction.reply({ content: "You do not have permission to run this command", ephemeral: true})
        ...
        await timeMember.timeout(duration * 1000, reason)
            ...
            .setDescription(`**${timeUser.tag}** has been **timed out** for **${duration / 60}** minutes in ${interaction.guild.name} \n \n **Reason:** ${reason}`)
            ...
            .setDescription(`You have been timed out in **${interaction.guild.name}** for **${duration / 60}** minutes in ${interaction.guild.name} \n \n **Reason:** ${reason}`)
            .setColor('Red')
            .setTimestamp()
